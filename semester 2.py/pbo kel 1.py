...
class Buku:
    def _init_(self, judul, pengarang, isbn, tahun, stok):
        self.judul, self.pengarang = judul, pengarang
        self.isbn, self.tahun = isbn, tahun
        self.stok, self.dipinjam = stok, 0
...
class Anggota:
    def _init_(self, nama, id_anggota, email):
        self.nama, self.id = nama, id_anggota
        self.email, self.pinjam = email, []
        self.maks_pinjam = 3
...
class Mahasiswa(Anggota):
    def _init_(self, nama, id_anggota, email, nim, jurusan):
        super()._init_(nama, id_anggota, email)
        self.nim, self.jurusan = nim, jurusan
        self.maks_pinjam = 5

class Dosen(Anggota):
    def _init_(self, nama, id_anggota, email, nip, fakultas):
        super()._init_(nama, id_anggota, email)
        self.nip, self.fakultas = nip, fakultas
        self.maks_pinjam = 10

class Peminjaman:
    def _init_(self, id_pinjam, anggota, buku, tgl_pinjam):
        self.id, self.anggota, self.buku = id_pinjam, anggota, buku
        self.tgl_pinjam, self.tgl_kembali = tgl_pinjam, None
...
class SistemPerpustakaan:
    ...
    def pinjam_buku(self, id_anggota, judul_buku, tgl):
        anggota, buku = self.cari_anggota(id_anggota), self.cari_buku(judul_buku)
...
def main():
    ...
    for m in mahasiswa: s.tambah_anggota(m)
